function [logp, yhat, res] = tapas_beta_reg_obs(r, infStates, ptrans)
...
%     mu = tapas_sgm(infStates(:,1), 1);
    ...
    sa1hat = infStates(:,1,2); % Default: predictions (ie, mu1hat)
    if r.c_obs.predorpost == 2
        mu1hat = tapas_sgm(infStates(:,2,3), 1); % Alternative: posteriors (ie, sgm(mu2))
    end
end
